...
class FakePlatform:
    ...
    async def post_json(self, path: str, payload: dict) -> dict:
        ...
        return {
            ...
            "openrouter_model_package": {"default_model": "deepseek/deepseek-v4-pro"},
        }
...
def test_telegram_network_fallback_persists_process_override_without_metadata() -> None:
...
def test_configure_telegram_writes_env_and_starts_gateway() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            with (
                ...
                patch(
                    ...
                    lambda token, **kwargs: (
                        menu_calls.append({"token": token, **kwargs})
        ...
        assert "shell_init_files:" not in config_text
        assert str(home / ".hermes" / "tinyhat" / "terminal-env.sh") not in config_text
    ...
    assert configure_telegram.DEFAULT_TELEGRAM_FALLBACK_IPS not in str(
        result["telegram_network"]
    )
    assert menu_calls == [
        {
    ...
    async def fake_run_process(
        ...
        *,
        ...
        if command == "start":
            ...
            command == "status"
            and len([call for call in gateway_calls if call[-1] == "status"]) == 1
        ):
...
def test_active_gateway_foreground_generation_rejects_pid_reuse_and_missing_state() -> (
    None
):
    generation = {
        "schema": configure_telegram.FOREGROUND_GATEWAY_STATE_SCHEMA,
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch.dict(
            ...
            with patch(
                ...
                reused = configure_telegram._active_gateway_foreground_generation(
                    Path("/usr/local/bin/hermes")
                )
            with patch(
                "hermes_runtime.gateway_readiness.read_gateway_runtime_generation",
                ...
                missing = configure_telegram._active_gateway_foreground_generation(
                    Path("/usr/local/bin/hermes")
                )
...
def test_install_codex_auth_plugin_commands_enables_menu_plugin() -> None:
    ...
    assert (
        result["mechanism"]
        == "hermes_plugin_register_command_and_transcription_provider"
    )
...
def test_codex_auth_plugin_source_compiles_and_maps_stt_errors() -> None:
    ...
    assert error_text == (
        "OpenAI Codex STT failed (429): billing_not_active: Billing is not active."
    )
    assert provider.name == "openai-codex-stt"
    assert provider.default_model() == "gpt-4o-transcribe"


def test_configure_codex_multimedia_keeps_openrouter_stt_and_sets_codex_vision() -> (
    None
):
    with tempfile.TemporaryDirectory() as tmp:
    ...
    assert any(call[-2:] == ["auxiliary.vision.model", "gpt-5.5"] for call in calls)
    assert keys == [
        "stt.enabled",
        "stt.provider",
        "stt.providers.openrouter.type",
...
def test_install_telegram_command_menu_priority_uses_hermes_config_shape() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        text = config.read_text(encoding="utf-8")
        retry_result = configure_telegram._install_telegram_command_menu_priority(
            config
        )
...
def test_telegram_setup_repairs_orphan_priorities_from_older_setup() -> None:
    try:
        import yaml
    except ImportError:
        raise unittest.SkipTest("YAML runs in the Hermes interpreter")
    source = (
        ...
        "plugins:\n  enabled:\n    - tinyhat-codex\n  - tinyhat\n  - tinyhat-codex\n"
        "platforms:\n  telegram:\n    extra:\n      command_menu:\n"
        f"        {configure_telegram.TELEGRAM_MENU_START_MARKER}\n"
        "        max_commands: 40\n        priority:\n          - tinyhat_settings\n"
        f"        {configure_telegram.TELEGRAM_MENU_END_MARKER}\n"
        "        - custom\n        - model\n"
        "agent:\n  max_turns: 75\n"
    )
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for _ in range(2):
            ...
            configure_telegram._install_telegram_command_menu_priority(path)
            parsed = yaml.safe_load(path.read_text())
            assert parsed["model"]["default"] == "owner/model"
            assert parsed["agent"]["max_turns"] == 75
            assert parsed["plugins"]["enabled"] == ["tinyhat-codex", "tinyhat"]
            menu = parsed["platforms"]["telegram"]["extra"]["command_menu"]
            assert menu["max_commands"] == 40
            assert "custom" in menu["priority"] and "model" in menu["priority"]
