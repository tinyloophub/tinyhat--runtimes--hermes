...
| `remove_private_secret` | `hermes_runtime/commands/remove_private_secret.py` | Removes one generation-bound Tinyhat secure credential after the owner confirms deletion. | Deletes the named assignment and terminal alias from Hermes env files, clears the current runtime process value, and requires a healthy generation-verified gateway restart before reporting `credential_removal_verified=true`. Results contain only the credential name, paths, and proof metadata; the value is never returned. |
