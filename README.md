...
| `list_hermes_secrets_masked` | `hermes_runtime/commands/list_hermes_secrets_masked.py` | Lists Tinyhat-managed secrets available from Hermes env files. | Read-only. Reads the Tinyhat-managed secret blocks in Hermes env files and reports secret names, source files, process availability, and masked values. It never returns plaintext secret values or unrelated env-file entries. |
