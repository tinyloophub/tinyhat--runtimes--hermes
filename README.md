...
| `start_google_workspace_reviewer_oauth` | `hermes_runtime/commands/start_google_workspace_reviewer_oauth.py` | Lets the platform ask one assessment reviewer flow to start the existing Computer-held Google Workspace OAuth handoff. | Accepts exactly one bounded opaque `reviewer_request_id` and calls only the installed Tinyhat plugin's documented `start_google_workspace_reviewer_oauth` function. The result is fixed and value-blind: it never returns or echoes an authorization URL, token, key, code, credential, owner token, or request id. The heartbeat advertises support at `hermes_runtime.capabilities.google_workspace_reviewer_oauth_start`. |
