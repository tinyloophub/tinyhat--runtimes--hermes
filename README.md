...
| `disconnect_slack` | `hermes_runtime/commands/disconnect_slack.py` | Revokes and removes one generation-bound Slack connection after owner confirmation. | Revokes an active bot token through Slack when possible, removes all local Slack values and terminal aliases together, and requires a healthy generation-verified Hermes restart before reporting `disconnect_verified=true`. Results contain only safe status and proof metadata. |
