...
| `apply_config` | `hermes_runtime/commands/apply_config.py` | Applies Tinyhat runtime config changes after the settings Mini App saves a secret. | Fetches the latest computer-authenticated runtime secret map, writes the values into Hermes env files, reloads the updated keys into the runtime process, registers the secret names with Hermes' terminal env passthrough config, sends the owner a short notice, and restarts `hermes gateway` so Hermes loads added, updated, or removed secret env vars. The command result includes only secret names and env-file paths, never secret values. |
