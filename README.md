...
| `apply_config` | `hermes_runtime/commands/apply_config.py` | Applies Tinyhat runtime config changes after the settings Mini App saves a secret. | Fetches the latest computer-authenticated runtime secret map, writes the values into Hermes env files, reloads the updated keys into the runtime process, and sends the owner a short availability notice. Current Hermes gateway builds reload `~/.hermes/.env` before each turn, so add/update applies do not restart `hermes gateway`; if a previously managed secret was removed, the command restarts the gateway after a restart notice so the removed env var is no longer inherited. The command result includes only secret names and env-file paths, never secret values. |
| `list_hermes_secrets_masked` | `hermes_runtime/commands/list_hermes_secrets_masked.py` | Lists masked secret/env names available from Hermes env files. | Read-only. Asks Hermes for its canonical env file with `hermes config env-path`, falls back to Tinyhat's known env-file candidates, and reports env names, source files, process availability, fixed masked values, and Tinyhat-managed block metadata when present. It never returns plaintext secret values. |
