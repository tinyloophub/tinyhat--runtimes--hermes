...
3. Runs the same idempotent Google Chrome Stable installer exposed by the
   `install_google_chrome` runtime command. Supported amd64 and arm64 Linux
   Computers therefore receive the familiar Chrome desktop browser during
   provisioning, while older Computers can be repaired later without
...
| `install_google_chrome` | `hermes_runtime/commands/install_google_chrome.py` | Installs the familiar Google Chrome Stable desktop browser on an existing Computer. | Runs the same idempotent helper used by `install.sh`. On supported apt-based amd64 and arm64 Linux Computers it downloads Google's official architecture-specific `.deb`, installs it with apt, and verifies the Chrome binary and version. Existing Chrome installations are preserved. It does not configure browser profiles or read browser data. |
