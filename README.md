...
| --- | --- |
| `/codex_auth` | Starts the official Codex CLI device-code auth flow in the background. The helper sends the authorization link as a Telegram button, sends the device code as a separate copyable message, waits for OpenAI to finish the device flow on this Computer, asks Hermes through its formal model picker to import/switch to OpenAI Codex, switches image understanding to the Codex/GPT vision provider, keeps OpenRouter Whisper STT active, registers `openai-codex-stt` settings only for opt-in accounts, sends a short restart notice, and restarts the Telegram gateway so the next reply uses the new model credential. |
