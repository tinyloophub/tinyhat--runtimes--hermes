...
| `apply_config` | `hermes_runtime/commands/apply_config.py` | Applies Tinyhat runtime config changes after the settings Mini App saves a secret. | Fetches the latest computer-authenticated runtime secret map, writes the values into Hermes env files, reloads the updated keys into the runtime process, records Tinyhat-managed terminal aliases for the saved names, sends the owner a short notice, and restarts `hermes gateway` so Hermes loads added, updated, or removed secret env vars. Command results include only secret names and env-file paths, never secret values. |
