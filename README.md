...
3. Runs the same idempotent desktop-app installer exposed by the
   `install_desktop_apps` runtime command. Supported amd64 and arm64 Linux
...
6. Writes runtime state under `/var/lib/tinyhat-hermes-runtime`.
7. Records the installed ref in `current/VERSION` and, when it can resolve one,
   the installed commit in `current/COMMIT_SHA`.
8. Writes a private env file at `/opt/tinyhat-hermes-runtime/env/runtime.env`.
   With systemd, the same env is copied to `/etc/tinyhat/hermes-runtime.env`.
9. When `--run-foreground` is passed, runs the installed runtime in the
   foreground with a small restart loop for local Docker or another external
   supervisor.
10. When systemd is enabled, installs a service named
...
| `install_hermes` | `hermes_runtime/commands/install_hermes.py` | Installs the pinned upstream Hermes Agent after the Tinyhat runtime is alive, and skips reinstalling when `hermes` already exists. | May install Debian prerequisites as root, then runs the official Hermes installer at the commit declared in `hermes_runtime/day_one_capabilities.py`. Browser installation is enabled; Tinyhat requires `hermes doctor` to report Playwright Chromium ready, then uses the documented browser CLI to open and snapshot a deterministic local `data:` page, record the Chromium and automation CLI versions plus executable location, and close the smoke session cleanly. The smoke proves browser readiness without treating transient internet or DNS failure as a broken Computer. When Linux x86 needs repair, it runs `agent-browser install --with-deps` and installs pinned Playwright Chromium so both the lower-level CLI and Hermes recognize the browser. It never preinstalls Ubuntu's snap Chromium on x86 and only falls back to distro Chromium on Linux ARM. The command installs and verifies the pinned Google Workspace CLI from Google's official statically linked Linux asset after checking its architecture-specific SHA-256; Workspace authentication remains a later user-controlled connection. It also installs the pinned `jmapc` client in Hermes' venv, exposes `/usr/local/bin/tinyhat-jmap-python`, and runs a real import/version smoke through that Computer-wide launcher. Assigned mailbox credentials can therefore be used directly from the Agent terminal without adding work to assignment. It pins and verifies DDGS search plus Edge TTS in that venv, applies the public day-one search/browser/image/TTS/STT/vision defaults, enables Telegram native rich messages with experimental rich drafts disabled, warms the selected local faster-whisper STT model cache (`small` by default; override with `TINYHAT_HERMES_LOCAL_STT_MODEL`, skip only with `TINYHAT_SKIP_LOCAL_STT_MODEL_PREFETCH=1`) for operator-selected local mode, then preinstalls the Codex auth quick commands plus the matching Hermes menu plugin in `~/.hermes/config.yaml` so a later Telegram connection is fast. Local STT prefetch failures are reported without failing provisioning because OpenRouter STT is the active day-one provider. Assignment remains separate: it writes the Computer's Telegram/OpenRouter credentials and starts the gateway after the Computer is selected for an agent. Results include the capability baseline/fingerprint and distinguish no-op from install: `installed_now` means this command ran the installer; `installed_after` means Hermes is present after the command. |
| `install_desktop_apps` | `hermes_runtime/commands/install_desktop_apps.py` | Installs the familiar Google Chrome Stable browser and Thunar file manager on an existing Computer. | Runs the same idempotent helper used by `install.sh`. On supported apt-based amd64 and arm64 Linux Computers it downloads Google's official architecture-specific Chrome `.deb`, installs Chrome and the distro's Thunar package with apt, and verifies both binaries and versions. Existing installations are preserved. It does not configure browser profiles or read browser or file data. |
...
| `configure_channels` | `hermes_runtime/commands/configure_channels.py` | Connects optional Telegram and Slack channels to an already-owned Computer. | Reads assignment-scoped platform configuration, publishes the Computer public key, decrypts Slack credentials locally, installs owner allowlists before tokens, and verifies each provider before acknowledgement. Already-applied revisions do not restart the gateway; when supported, optional `slack_identity` readback reports public chat identifiers. Missing or stale metadata reports leave working channels intact. Keeps model, email, and user files. Requires a compatible Tinyhat plugin with the standard-library-only `capabilities/channels/runtime.py` adapter. |
...
| `remove_private_secret` | `hermes_runtime/commands/remove_private_secret.py` | Removes one generation-bound Tinyhat secure credential after the owner confirms deletion. | Deletes the named assignment and terminal alias from Hermes env files, clears the current runtime process value, and requires a healthy generation-verified gateway restart before reporting `removal_verified=true`. Results contain only the env name, paths, and proof metadata; the value is never returned. |
| `configure_hat_credentials` | `hermes_runtime/commands/configure_hat_credentials.py` | Starts first-time or replacement grouped credential entry from a Hat preview without asking the user to prompt the agent manually. | Calls the installed Tinyhat plugin on the Computer, which creates or reuses the Hat-local key pair and sends one encrypted-entry button in Telegram. Values remain Computer-local and Hermes is not restarted. |
| `resume_hat_installation` | `hermes_runtime/commands/resume_hat_installation.py` | Resumes the already-authorized Hat installation on an assigned Computer without waiting for a user message or a Hermes/model turn. | Calls the installed Tinyhat plugin's deterministic `resume_installation` action. The runtime validates the plugin result and returns only the installation status, a normalized outcome, and whether installation started. Missing Hats are quiet no-ops; identifiers, paths, credential data, and unrestricted plugin output never cross the command boundary. |
| `complete_hat_credential_transfer` | `hermes_runtime/commands/complete_hat_credential_transfer.py` | Lets the platform automatically request one already-authorized Hat credential delivery from the exact creator Computer, without a creator chat or LLM turn. | Calls the installed Tinyhat plugin for one bounded handoff id and canonical Hat handle. The plugin encrypts to the consumer key and signs locally; the runtime returns only an allowlisted count/identifier result and never receives plaintext or ciphertext. |
...
new assignments keep OpenRouter Whisper STT active unless the operator opts
into the Codex STT provider. The command bodies still call the runtime helpers below; the
...
| --- | --- |
| `/codex_auth` | Starts fresh, separate official Codex CLI and Hermes device-code sign-ins in the background, even when saved credentials report logged in. The helper sends the authorization link as a Telegram button, sends the device code as a separate copyable message, waits for OpenAI to finish the device flow on this Computer, then asks Hermes through its formal model picker to switch to OpenAI Codex, configures image understanding to use the selected Codex chat model by default with OpenRouter as fallback, keeps OpenRouter Whisper STT active, registers `openai-codex-stt` settings only for opt-in accounts, sends a short restart notice, and restarts the Telegram gateway so the next reply uses the new model credential. |
