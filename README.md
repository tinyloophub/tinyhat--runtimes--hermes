...
| `install_hermes` | `hermes_runtime/commands/install_hermes.py` | Installs upstream Hermes Agent after the Tinyhat runtime is alive, and skips reinstalling when `hermes` already exists. | May install Debian prerequisites as root, then runs `curl -fsSL https://hermes-agent.nousresearch.com/install.sh \| bash`. By default Tinyhat passes `--skip-browser`; set `TINYHAT_HERMES_INSTALL_ARGS` to override. Verifies Hermes messaging and voice dependencies, applies the day-one OpenRouter multimodal defaults, warms the selected local faster-whisper STT model cache (`small` by default; override with `TINYHAT_HERMES_LOCAL_STT_MODEL`, skip only with `TINYHAT_SKIP_LOCAL_STT_MODEL_PREFETCH=1`) for operator-selected local mode, then preinstalls the Codex auth quick commands plus the matching Hermes menu plugin in `~/.hermes/config.yaml` so a later Telegram connection is fast. Local STT prefetch failures are reported without failing provisioning because OpenRouter is the active day-one STT provider. The prerequisite set includes `ffmpeg` for Telegram TTS voice bubbles, `ripgrep` for fast search, `build-essential` for native modules, and `xclip`/`wl-clipboard` for Linux desktop image paste. Result fields distinguish no-op from install: `installed_now` means this command ran the installer; `installed_after` means Hermes is present after the command. |
...
| `heal_hermes` | `hermes_runtime/commands/heal_hermes.py` | Repairs an already-configured Hermes Computer whose messaging gateway stopped while the Tinyhat runtime still heartbeats. | Verifies Hermes is installed, confirms Tinyhat-managed Telegram env names are present without returning values, reloads env files, and by default reuses the durable `start_hermes` gateway path (start-only; no-ops on a healthy gateway). With `spec.restart=true` it performs a durable one-shot gateway restart via the official `hermes gateway restart` command (owned end-to-end by the runtime process): reload env files, run `hermes gateway restart`, then poll functional readiness — `hermes gateway status` healthy plus best-effort Telegram connect evidence from the gateway log or `journalctl` — until `spec.deadline_seconds` (default 90, clamped 30–300). The result's `restart` object reports `{requested, performed, deadline_seconds, deadline_exceeded, telegram_evidence: log\|journal\|unavailable, milestones_ms}`, and `healthy` is true only when the restart command succeeded AND readiness verified within the deadline (a failed `hermes gateway restart` reports `gateway_restart_command_failed`, never a false success). The runtime never queues this on its own beyond the single assignment-time bring-up; restart policy belongs to the platform. If Telegram is not configured it reports `telegram_not_configured` instead of fetching new bot credentials. Assigned heartbeats separately include a cheap status-only gateway readiness snapshot in `metrics.hermes_runtime.gateway`. |
