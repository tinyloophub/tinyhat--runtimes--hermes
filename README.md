...
| `install_hermes` | `hermes_runtime/commands/install_hermes.py` | Installs upstream Hermes Agent after the Tinyhat runtime is alive, and skips reinstalling when `hermes` already exists. | May install Debian prerequisites as root, then runs `curl -fsSL https://hermes-agent.nousresearch.com/install.sh \| bash`. By default Tinyhat passes `--skip-browser`; set `TINYHAT_HERMES_INSTALL_ARGS` to override. Verifies Hermes messaging and voice dependencies, warms the selected local faster-whisper STT model cache (`small` by default; override with `TINYHAT_HERMES_LOCAL_STT_MODEL`, skip only with `TINYHAT_SKIP_LOCAL_STT_MODEL_PREFETCH=1`), then preinstalls the Codex auth quick commands plus the matching Hermes menu plugin in `~/.hermes/config.yaml` so a later Telegram connection is fast. The prerequisite set includes `ffmpeg` for Telegram TTS voice bubbles, `ripgrep` for fast search, `build-essential` for native modules, and `xclip`/`wl-clipboard` for Linux desktop image paste. Result fields distinguish no-op from install: `installed_now` means this command ran the installer; `installed_after` means Hermes is present after the command. |
