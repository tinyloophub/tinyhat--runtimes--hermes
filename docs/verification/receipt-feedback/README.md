...
- Telegram accepted `sendChatAction` 0.155 seconds after local ingest. Telegram
  Desktop visibly showed typing. Saving the quiet preference stopped renewal.
- Slack accepted `agents.sessions.setStatus` 0.249 seconds after local ingest.
...
status writes, local quiet preferences, and router preference restrictions.
